...
document.getElementById('btn-confirmar-corrida')?.addEventListener('click', async () => {
  ...
  state.pontoReferencia = (inputReferencia?.value || '').trim();
  ...
  exibirPontoReferencia(state.pontoReferencia);
...
// ─────────────────────────────────────
// PONTO DE REFERÊNCIA + CONTADOR DE ESPERA
// ─────────────────────────────────────
let esperaTimerInterval = null;
let configEsperaCache = { tempoGratuitoMin: 3, taxaEsperaMin: 0.50 };

async function carregarConfigEspera() {
  if (!firebaseReady || !db) return configEsperaCache;
  try {
    const snap = await fb.getDoc(fb.doc(db, 'configuracoes', 'app'));
    const c = snap.exists() ? (snap.data()?.corridas || {}) : {};
    const tempo = Number(c.tempoGratuitoMin);
    const taxa = Number(c.taxaEsperaMin);
    if (tempo > 0) configEsperaCache.tempoGratuitoMin = tempo;
    if (taxa >= 0) configEsperaCache.taxaEsperaMin = taxa;
  } catch (e) { console.warn('[passageiro] config de espera indisponível:', e); }
  return configEsperaCache;
}

function exibirPontoReferencia(texto) {
  const el = document.getElementById('tracking-referencia');
  if (!el) return;
  const v = (texto || '').trim();
  el.style.display = v ? 'block' : 'none';
  el.textContent = v ? ('📍 Referência: ' + v) : '';
}

function pararContadorEspera() {
  if (esperaTimerInterval) clearInterval(esperaTimerInterval);
  esperaTimerInterval = null;
}

async function iniciarContadorEspera(inicioMs) {
  pararContadorEspera();
  const cfg = await carregarConfigEspera();
  const card = document.getElementById('espera-contador-card');
  const contador = document.getElementById('espera-contador');
  const status = document.getElementById('espera-status');
  if (!card || !contador || !status) return;
  card.hidden = false;
  const inicio = Number(inicioMs) || Date.now();
  const gratisMs = Math.max(1, Number(cfg.tempoGratuitoMin || 3)) * 60 * 1000;
  const tick = () => {
    const decorrido = Math.max(0, Date.now() - inicio);
    const restante = gratisMs - decorrido;
    if (restante > 0) {
      const s = Math.ceil(restante / 1000);
      contador.textContent = String(Math.floor(s/60)).padStart(2,'0') + ':' + String(s%60).padStart(2,'0');
      status.textContent = 'Tempo gratuito';
    } else {
      const excedenteMs = Math.abs(restante);
      const s = Math.floor(excedenteMs/1000);
      contador.textContent = '+' + String(Math.floor(s/60)).padStart(2,'0') + ':' + String(s%60).padStart(2,'0');
      const minCob = Math.ceil(excedenteMs/60000);
      const valor = minCob * Number(cfg.taxaEsperaMin || 0);
      status.textContent = 'Após tolerância · estimativa de espera: R$ ' + valor.toFixed(2).replace('.', ',');
    }
  };
  tick();
  esperaTimerInterval = setInterval(tick,1000);
}
...
async function criarCorrida(origem, destino, preco, categoria, precoOriginal) {
  ...
  const corridaLocal = {
    ...
    pontoReferencia: (state.pontoReferencia || '').trim() || null,
    origemLat,
    origemLon,
    destinoLat,
    destinoLon,
...
function ouvirAceiteCorrida(corridaId) {
  ...
  state.corridaListenerUnsub = fb.onSnapshot(fb.doc(db, 'corridas', corridaId), (snap) => {
    ...
    if (!data) return;
    if (data.pontoReferencia) exibirPontoReferencia(data.pontoReferencia);
    if (data.motoristaChegou && !window._jaAvisouChegouPass) {
      window._jaAvisouChegouPass = true;
      alertaNativo('chegou');
      showToast('📍 Seu motorista chegou!');
      const chegouEm = data.motoristaChegouEm?.toMillis ? data.motoristaChegouEm.toMillis() : (Number(data.motoristaChegouEm) || Date.now());
      iniciarContadorEspera(chegouEm);
    }
